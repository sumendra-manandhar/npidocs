...
## 2.3	Transaction Flow SMS

Single Message System (SMS) encompasses a transaction that is transmitted only once for authorization, clearing and settlement, and is also called a comprehensive financial transaction, i.e. authorization, clearing, and settlement all take place online. All domestic ATM transactions are managed through SMS. SMS environment uses a single exchange of transaction message between the acquirer and the issuer to:
	Authorize a financial transaction
	Post a financial transaction to a cardholder’s account
The National Card Switch SMS system will perform real time transaction processing as well as exception or offline transaction processing offline. Transaction flow in SMS environment is as follows:
  
![Example Image](/img/sms_TXN.jpg)
<p align="center" class="centered-caption"></p>

  Figure 2: SMS Transaction Flow



## 2.4	Transaction Flow DMS
...
![Example Image](/img/dms_txn.jpg)
<p align="center" class="centered-caption"></p>

Figure 3: DMS Transaction Flow
...
## 2.5	Transaction Routing

Transaction Routing is the process of moving transactions from a source member participant to a destination member participant. The National Card Switch supports routing of interbank POS and ATM transactions. 

1.	The National Card Switch will support the routing for authorization for both SMS and DMS system. The clearing and settlement of DMS transaction is carried through Real Time Gross Settlement (RTGS) system.


2.	The National Card Switch validates the request message from the acquirer and prepares it for processing. This processing and validation include identifying the message type, identifying the Issuer, checking of structural, format and value validation.

If the National Card Switch encounters an error condition at any point in the process, then further processing is halted. Messages rejected or declined by NPS are sent back to the acquirer with a proper response code indicating occurrence of an error condition wherever possible and the message is not forwarded to the issuer. For e.g. if a message does not contain a mandatory field in the request, or a field contains an alphabet in place of a number then that message would be rejected at the NPS’s end.
